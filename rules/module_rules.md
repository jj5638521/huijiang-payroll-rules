...
版本：v2026-02-11R54.1-MealFormSingleSelectSchemaCompat
版本序号：#54.1
阻断模式：Hard
...
- 施工说明相关列属于施工表字段，不参与 PaymentPipe 关键词命中（例如“已结清”仍仅在 PaymentPipe 文本列识别）。
- 表头清洗后字段映射（取首个非空列）：
  - 日期 ← 日期 | 施工日期 | 工作日期
  ...
  - 施工说明/问题描述/其他说明（同义映射补充） ← `施工说明` | `问题描述` | `其他说明` | `报销说明` | `摘要` | `备注`（仅限施工表内 AttendancePipe 可见列）

- 表头清洗补充（仅用于匹配，不改源表）：
  - 去除空白字符（空格/制表符）、全角半角差异与隐藏换行后再匹配。
  - 对“问题描述（如设备损坏、特殊事项等）”等括号扩展名，匹配时允许以关键词“问题描述”命中。
